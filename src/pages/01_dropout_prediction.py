...
def render_xgb_input_form(df_ref: pd.DataFrame, defaults: dict, form_key: str):
    ...
    with st.form(form_key):
        if mode == "간소화 입력(추천)":
            st.caption("XGBoost 핵심 피처 5개만 입력합니다. 나머지는 기본값으로 자동 입력됩니다.")
            c1, c2 = st.columns(2)
            with c1:
                code_module = st.selectbox(
                    ...
                    key=f"{form_key}_code_module_simple",
                )
                code_presentation = st.selectbox(
                    ...
                    key=f"{form_key}_code_presentation_simple",
                )
...
def normalize_model_name(model_name: str) -> str:
    ...
    if "random_forest_live" in name:
        return "Random Forest (Live)"
    if "random_forest" in name:
        return "Random Forest"
    if "tabnet" in name:
        return "TabNet"
    if "xgboost_live" in name:
        return "XGBoost (Live)"
    if "xgboost" in name:
        return "XGBoost"
...
def save_live_prediction(model_name, id_student, pred, prob, run_id):
    ...
    df_save.to_sql("predictions", engine, if_exists="append", index=False)
...
def rf_inference_ui(df_ref: pd.DataFrame):
    ...
    if submitted:
        ...
        if save_prediction:
            ...
            save_live_prediction("random_forest_live", id_student, pred, prob, rf_run_id)
        ...
        if save_prediction:
            ...
            if user_id_text.strip():
                if not user_id_text.strip().isdigit():
                    ...
                    st.stop()
                id_student = int(user_id_text.strip())
            save_live_prediction("logistic_live", id_student, pred, prob, run_id)
            st.success("예측 결과를 저장했습니다.")
        ...
        if save_prediction:
            ...
            if user_id_text.strip():
                if not user_id_text.strip().isdigit():
                    ...
                    st.stop()
                id_student = int(user_id_text.strip())
            save_live_prediction("xgboost_live", id_student, pred, prob, run_id)
            st.success("예측 결과를 저장했습니다.")
...
with tab_analysis:
    ...
    # Logistic 실시간 저장분 제외 + 과거 명칭(logistic_regression) 중복 방어
    model_name_norm = df_pred["model_name"].astype(str).str.lower().str.strip()
    df_pred = df_pred[model_name_norm != "logistic_live"]
    # logistic_dropout이 있으면 해당 값을 공식값으로 사용하고 logistic_regression은 제외
    if (model_name_norm == "logistic_dropout").any():
        df_pred = df_pred[model_name_norm != "logistic_regression"]
    df_pred["display_model"] = df_pred["model_name"].apply(normalize_model_name)

    models = sorted(df_pred["display_model"].dropna().unique().tolist())
    ...
    if len(models) > 1:
        rows = []
        for m in sorted(df_pred["display_model"].dropna().unique().tolist()):
