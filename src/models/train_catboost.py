"""
Student Dropout — CatBoost + SHAP + MLflow
Flow:
  1. students + studentVle 로드 (MySQL, 청크 방식)
  2. studentVle CUTOFF_DAY 기준 집계
  3. 전처리 & 합치기
  4. 전처리 데이터 MySQL 업로드
  5. CatBoost 학습 (RandomizedSearchCV 튜닝)
  6. MLflow 로깅 & 모델 등록
...
from sqlalchemy import create_engine, Float, Integer, SmallInteger, String
...
EXPERIMENT   = 'catboost_stu_vle'
MODEL_NAME   = 'catboost_dropout'
OUTPUT_TABLE = 'preprocessed_students'
RANDOM_STATE = 42
TEST_SIZE    = 0.2
CUTOFF_DAY   = 60
OUTPUT_DIR   = '/home/ict/work/outputs/catboost'
...
# ── 4. 전처리 ─────────────────────────────────────────────────────────────────
# CatBoost는 범주형을 직접 처리할 수 있어서 인코딩 최소화
TARGET    = 'dropout'
...
def preprocess(df: pd.DataFrame):
    ...
    # CAT_COLS는 문자열로 유지 (CatBoost가 직접 처리)
    for col in CAT_COLS:
        if col in df.columns:
...
candidates = []
for _ in range(20):
    candidates.append({k: random.choice(param_dist[k]) for k in keys})
...
for i, params in enumerate(candidates):
    model = CatBoostClassifier(
        **params,
        ...
        verbose=0,
        train_dir=tempfile.mkdtemp(),  # ← catboost_info 폴더 생성 방지
    )
    model.fit(X_train, y_train)
    y_prob_cv = model.predict_proba(X_test)[:, 1]
    score = f1_score(y_test, (y_prob_cv >= 0.5).astype(int))
    print(f'  [{i+1}/20] F1: {score:.4f} | params: {params}')
    if score > best_score:
        best_score = score
        best_model = model
        best_params = params
...
with mlflow.start_run(run_name='catboost') as run:
    ...
    mlflow.log_params({**best_params, 'cutoff_day': CUTOFF_DAY})
    ...
    ax.set_title(f'Confusion Matrix - CatBoost (cutoff={CUTOFF_DAY}일)')
    ...
    # 모델 저장 (네이티브 포맷)
    model_path = f'{OUTPUT_DIR}/catboost_model.cbm'
