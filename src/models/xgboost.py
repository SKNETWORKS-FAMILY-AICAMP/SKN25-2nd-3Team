...
# `python src/models/xgboost.py` 직접 실행 시, 로컬 파일명이 라이브러리명과 같아
...
def train(engine=None, cutoff_day: int = CUTOFF_DAY):
    ...
    out_dir = os.path.join(os.path.dirname(__file__), "..", "..", "outputs", "xgboost")
    ...
    with mlflow.start_run(run_name="xgboost") as run:
        ...
        ax.set_title(f"Confusion Matrix - XGBoost (cutoff={cutoff_day})")
        cm_path = os.path.join(out_dir, "confusion_matrix.png")
        fig.savefig(cm_path, bbox_inches="tight")
        plt.close(fig)
        mlflow.log_artifact(cm_path)

        # SHAP은 실패 가능성이 있어 try/except 처리
        try:
