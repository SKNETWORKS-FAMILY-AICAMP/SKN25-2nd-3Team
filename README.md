...
| Docker 환경 기반의 DBeaver, MLflow 연동 등 실무 수준의 파이프라인을 경험하며 엔지니어로서의 역량을 키웠고, 좋은 팀원들과 함께 데이터 기반의 인사이트를 도출해낼 수 있어 영광이었습니다. | Tailscale로 포트포워딩 없이 외부 접속을 구성해본 첫 경험이 흥미로웠고, F1-score 향상을 위해 피처 엔지니어링과 모델 비교 실험(XGBoost·CatBoost)을 진행했지만 범주형 처리에 강점이 있는 CatBoost도 이 데이터셋에서는 큰 차이를 보이지 않아 인상적이었던 프로젝트였습니다. | 간단한 팀 프로젝트일 거라 생각했는데 수업에서 배웠던 도커 환경 구축부터 Jupyter, DBeaver, MLflow, Streamlit까지 전반적인 개발 환경을 프로젝트에서 다뤄보며 한 단계 성장할 수 있었던 값진 경험이었습니다. GPT의 도움을 받았지만 모델 성능을 스스로 검증하는 것도 의미 있었습니다. | 도커로 개발 환경을 통일해 협업을 진행하면서 초기에는 접속·연결 오류를 많이 겪었지만, 이를 해결하는 과정에서 환경 구성에 대한 이해가 크게 높아졌고, 모델링과 데이터 전처리를 직접 구현하며 하이퍼파라미터 튜닝으로 F1-score를 개선해본 경험까지 쌓을 수 있어 의미 있는 프로젝트였다. | Tailscale을 활용한 원격 서버 구축을 시작으로, MySQL과 MLflow 서버를 연동하고 이를 Streamlit과 연결하는 전체 파이프라인 환경을 구성하는 데 집중했다. 네트워크 설정, 포트 바인딩, 컨테이너 간 연결 구조를 직접 구성하면서 인프라 레벨의 흐름을 이해할 수 있었고, 단순 모델 학습을 넘어 실험 관리와 서비스 연결까지 이어지는 구조를 경험한 점이 의미 있었다. |
